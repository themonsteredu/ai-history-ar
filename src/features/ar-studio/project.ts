...
export function isStudioProject(v: unknown): v is StudioProject {
  ...
  return p.version === 1 && Number.isInteger(p.group) && p.group >= 1 && p.group <= 6 && Number.isInteger(p.heritageId) && p.heritageId >= 1 && p.heritageId <= 6 && typeof p.modelChecked === 'boolean' && typeof p.pointsChecked === 'boolean' && typeof p.role === 'string' && p.role.length <= 300 && typeof p.reflection === 'string' && p.reflection.length <= 1000 && isArExhibit(p.ar) && (p.ar.model?.format === 'primitives' || (p.ar.model?.format === 'preset' && preparedHeritage(p.ar.model.preset)?.id === p.heritageId) || (p.ar.model?.asset === 'cheomseongdae-nsm-2015' && p.heritageId === 3)) && p.ar.points.length >= 3 && Array.isArray(p.questions) && p.questions.length >= 2 && p.questions.length <= 3 && new Set(p.questions.map(q => q?.id)).size === p.questions.length && p.questions.every(q => q && typeof q.id === 'string' && q.id.length > 0 && q.id.length <= 60 && typeof q.prompt === 'string' && q.prompt.length <= 300 && Array.isArray(q.options) && q.options.length === 3 && q.options.every(o => typeof o === 'string' && o.length <= 150) && Number.isInteger(q.answer) && q.answer >= 0 && q.answer <= 2 && p.ar.points.some(point => point.id === q.pointId));
}
