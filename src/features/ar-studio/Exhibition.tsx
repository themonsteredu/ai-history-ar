...
import { cardSlots, groupQuizzes, quizProgress, waitingCards } from './visiting';
...
export function IndividualQuiz({ classroom, session, sound }: { classroom?: Classroom; session?: StudioSession; sound: ExhibitionSound }) {
  ...
  const sets = groupQuizzes(questions, answers, session?.group);
  const progress = quizProgress(questions, answers);
  ...
  return <section><header><h1>우리 반 전시 퀴즈</h1><p>관람한 모둠의 문제를 풀어요. 해설을 다시 들어도 괜찮아요.</p></header>{!session || !['quiz', 'review'].includes(classroom?.phase || '') ? <p>선생님이 퀴즈를 시작하면 문제가 나타납니다.</p> : <>{result?.submitted && <p className="studio-notice">{result.score === undefined ? '답안을 제출했어요. 선생님이 정답을 공개하면 확인할 수 있어요.' : `${result.total}문제 중 ${result.score}문제를 맞혔어요. 아래에서 해설의 근거를 확인해요.`}</p>}{!result?.submitted && progress.total > 0 && <p className="studio-quiz-progress" role="status">모둠 {sets.filter(set => set.answered === set.questions.length).length} / {sets.length} 완료 · 문제 {progress.answered} / {progress.total} 선택</p>}{sets.map(set => <section className="studio-quiz-group" key={set.group}>
    <h2>{set.group}모둠 문제{set.mine && <span className="studio-quiz-mine">우리 모둠</span>}<small>{set.answered} / {set.questions.length} 선택</small></h2>
    {set.questions.map((q, index) => { const detail = result?.details?.find(d => d.id === q.id); return <fieldset className="studio-question-editor" key={q.id}><legend>{index + 1}번 문제</legend><h3>{q.prompt}</h3><div className="studio-quiz-options">{q.options.map((option, i) => <button disabled={busy || !!result?.submitted || classroom?.phase === 'review'} aria-pressed={(detail?.chosen ?? answers[q.id]) === i} key={i} onClick={() => setAnswers(old => ({ ...old, [q.id]: i }))}>{i + 1}. {option}</button>)}</div><button onClick={() => { void listen(q.group, q.pointId); }}>이 문제의 해설 다시 듣기</button>{detail && <p>{detail.correct ? '맞았어요.' : '다시 확인해요.'} 정답: {detail.answer + 1}번</p>}</fieldset>; })}
  </section>)}{hint && <aside className="studio-hint"><h3>{hint.title}</h3><p>{hint.text}</p>{hint.narration && <audio ref={hintAudio} src={hint.narration.data} controls onPlaying={() => sound.narration(true)} onPause={() => sound.narration(false)} onEnded={() => sound.narration(false)} onError={() => sound.narration(false)} />}<button onClick={() => { hintAudio.current?.pause(); setHint(undefined); sound.narration(false); }}>해설 닫기</button></aside>}{!result?.submitted && classroom?.phase === 'quiz' && <div className="studio-reflection"><label>내가 맡은 일<input maxLength={300} value={role} onChange={e => setRole(e.target.value)} placeholder="예: 모형 만들기, 해설 녹음, 문제 작성" /></label><label>내가 확인하거나 배운 점<textarea maxLength={1000} rows={2} value={reflection} onChange={e => setReflection(e.target.value)} /></label><p>{progress.answered} / {progress.total}문제 선택{!progress.done && ' · 모든 모둠의 문제를 풀어야 제출할 수 있어요.'}</p><button className="studio-primary" disabled={busy || !questions.length || questions.some(q => !Number.isInteger(answers[q.id])) || !role.trim() || !reflection.trim()} onClick={() => { void submit(); }}>{busy ? '제출 중…' : '내 답안과 활동 기록 제출'}</button></div>}</>}{error && <p role="alert">{error}</p>}</section>;
}
