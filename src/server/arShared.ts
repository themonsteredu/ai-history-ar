...
export function createArSharedHandler(config: ArSharedConfig) {
  ...
  return async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      const response = await fetcher(`${config.url}/rest/v1/rpc/history_ar_dispatch`, { method: 'POST', signal: AbortSignal.timeout(20_000),
        headers: { 'Content-Type': 'application/json', apikey: config.key, Authorization: `Bearer ${config.key}` },
        body: JSON.stringify({ p_action: action, p_code: code, p_token_hash: await hash(token), p_payload: payload }),
      });
