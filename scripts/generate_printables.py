...
def three_kingdoms_data_sheet(era, lesson, width, st):
    ...
    if lesson_id == 1:
        questions = [["고르기", "관찰할 유산", "사진에서 찾기"]] + [["□", group["heritage"], "□ 모양  □ 무늬  □ 재료"] for group in era["groups"]]
        ...
        flow += [ruled_box("마지막 점검", "□ 원본을 남겼다   □ 빈칸을 추측하지 않았다\n□ 같은 뜻의 표기를 맞췄다   □ CSV로 저장했다", width, 62 * mm, st)]
    elif lesson_id == 6:
        rows = [["고를 것", "선택지", "확인"], ["그래프", "□ 막대  □ 점  □ 선", "□"], ["가로축", "□ 시기  □ 지역  □ 종류", "□"], ["세로축", "□ 개수  □ 비율", "□"], ["제목", "□ 있음", "□"], ["단위", "□ 있음", "□"]]
        flow += [data_table(rows, [width * .2, width * .65, width * .15], st, row_heights=[10 * mm] + [20 * mm] * 5), Spacer(1, 5 * mm)]
        flow += [ruled_box("그래프에서 찾아 동그라미", "가장 큰 값  ·  가장 작은 값  ·  비슷한 값", width, 55 * mm, st), Spacer(1, 4 * mm)]
        flow += [ruled_box("저장 확인", "□ 축 이름   □ 제목   □ 단위   □ PNG 저장", width, 42 * mm, st)]
    elif lesson_id == 7:
        flow += [sheet_columns([ruled_box("보이는 점 하나 고르기", "□ 많다  □ 적다  □ 비슷하다\n그래프에서 그 부분을 가리킵니다.", width * .47, 82 * mm, st)], [ruled_box("모르는 점 하나 고르기", "□ 자료가 적다\n□ 한 지역에 치우쳤다\n□ 연도가 정확하지 않다", width * .47, 82 * mm, st)], width, st), Spacer(1, 4 * mm)]
        flow += [ruled_box("말하기 문장틀", "‘__________이/가 가장 많습니다. 하지만 □ 자료가 적어  □ 한 지역에 치우쳐 전체 모습은 알 수 없습니다.’", width, 56 * mm, st)]
    elif lesson_id == 8:
        rows = [["고르기", "근거 카드", "확인"], ["근거 1", "□ 여러 지역에서 나옴  □ 같은 재료가 보임", "□"], ["근거 2", "□ 모양이 비슷함  □ 함께 나온 물건이 다름", "□"]]
        flow += [data_table(rows, [width * .18, width * .64, width * .18], st, row_heights=[10 * mm] + [34 * mm] * 2), Spacer(1, 5 * mm)]
        flow += [ruled_box("빈칸 하나만 채우기", "두 근거를 함께 보면, 옛사람들은 ______________________________했을 가능성이 있습니다.", width, 68 * mm, st), Spacer(1, 4 * mm)]
        flow += [ruled_box("단정하지 않기", "□ ‘반드시’라고 말하지 않음   □ ‘가능성이 있다’라고 말함", width, 48 * mm, st)]
    elif lesson_id == 9:
        evidence_rows = [["그래프 카드", "역사 사실 카드", "아직 모름 카드"], ["□ 많다\n□ 적다\n□ 비슷하다", "□ 자료 1\n□ 자료 2\n□ 자료 3", "□ 자료가 적다\n□ 의견이 나뉜다\n□ 원본이 없다"]]
        flow += [data_table(evidence_rows, [width * .33, width * .34, width * .33], st, row_heights=[12 * mm, 72 * mm]), Spacer(1, 5 * mm)]
        design_rows = [["AR에서 고르기", "선택"], ["가리킬 특징", "□ 모양  □ 무늬  □ 재료"], ["움직임", "□ 확대  □ 회전  □ 표시"], ["관람객 행동", "□ 찾기  □ 고르기  □ 확대"]]
        flow += [data_table(design_rows, [width * .28, width * .72], st, row_heights=[10 * mm] + [20 * mm] * 3), Spacer(1, 4 * mm)]
        flow += [ruled_box("30초 말하기 카드", "□ 그래프를 가리킴   □ 역사 사실을 말함   □ ‘아직 모름’을 말함", width, 50 * mm, st)]
    else:
        ...
        flow += [ruled_box("도슨트 말하기 카드", "□ 출처   □ 그래프   □ 유산 사실   □ ‘아직 모름’", width, 32 * mm, st), Spacer(1, 3 * mm)]
        flow += [ruled_box("수업을 마치며", "□ 자료 확인   □ 그래프 읽기   □ AI 답 다시 확인", width, 28 * mm, st)]
...
def activity_answer_examples(era, lesson):
    ...
    if is_three:
        data_examples = {
            1: [("학생 응답", "유산·사진 단서·질문 종류를 고르면 됩니다."), ("예시", "무령왕릉 + 발견 장소 + ‘어디에서 발견됐을까?’"), ("말하기", "질문을 쓰게 하지 않고 모둠에서 소리 내어 말하게 합니다.")],
            2: [("학생 응답", "여섯 문장마다 ‘자료와 맞음·확인 필요’ 중 하나를 고릅니다."), ("이유 카드", "출처 없음·너무 확실함·시대 불일치 중 가장 가까운 하나를 고릅니다."), ("답 공개", "학생 선택 뒤 교사가 다음 화면에서 근거를 공개합니다.")],
            3: [("검증 5단계", "출처·시기·교차·원본·보류를 확인할 때마다 체크합니다."), ("자료 상태", "확인·주의·보류 중 하나를 고릅니다."), ("마지막 선택", "근거가 부족하면 ‘아직 모름’을 고르는 것도 올바른 답입니다.")],
            4: [("핵심 낱말 예시", "백제 금동대향로: 6~7세기 / 백제 / 향로 / 국립부여박물관"), ("AI 설명", "‘모든 동물의 뜻이 밝혀졌다’는 말은 ‘과장’에 체크합니다."), ("아직 모름", "확인되지 않은 상징의 뜻은 ‘아직 모름’에 체크합니다."), ("출처", "웹 주소를 길게 옮기지 않고 교사가 붙인 자료 번호만 적습니다.")],
            5: [("중복", "같은 자료가 두 번 있으면 중복에 체크하고 하나만 남깁니다."), ("빈칸", "추측해 채우지 않고 빈칸에 체크합니다."), ("표기", "같은 뜻은 학급 표준표에 맞추고 표기에 체크합니다."), ("완료", "원본 보관·추측 안 함·표기 통일·CSV 저장을 확인합니다.")],
            ...
            7: [("보이는 점", "많다·적다·비슷하다 중 하나를 고릅니다."), ("모르는 점", "자료가 적다·한 지역에 치우쳤다·연도가 정확하지 않다 중 하나를 고릅니다."), ("발표", "고른 말을 문장틀에 넣어 말하면 됩니다.")],
            ...
            9: [("그래프 카드", "많다·적다·비슷하다 중 하나를 고릅니다."), ("역사 사실 카드", "교사가 번호를 붙인 공식 자료 카드 하나를 고릅니다."), ("AR 표현", "모양·무늬·재료와 확대·회전·표시 중 필요한 것을 고릅니다."), ("말하기", "그래프 → 역사 사실 → 아직 모름 카드를 보며 30초 동안 말합니다.")],
            10: [("전시 해설", "출처·그래프·유산 사실·아직 모름의 네 카드만 보고 말합니다."), ("AR 실패", "그래프 PNG와 QR 대체 자료로 설명을 계속합니다."), ("평가", "긴 글 대신 관람객 스탬프와 교사 관찰로 확인합니다.")],
        }
