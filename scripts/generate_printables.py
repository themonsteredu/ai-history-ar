...
def three_kingdoms_data_sheet(era, lesson, width, st):
    ...
    if lesson_id == 1:
        ...
        flow += [sheet_columns(
            [data_table(evaluation_rows, [left_width * .34, left_width * .66], st, row_heights=[10 * mm] + [21 * mm] * 4)],
            [data_table(planning_rows, [right_width * .42, right_width * .58], st, row_heights=[10 * mm] + [17 * mm] * 5)],
            width, st, left_ratio=.46,
        ), Spacer(1, 5 * mm)]
        flow += [ruled_box("AR 카드와 등장 장면 스케치", "카드의 굵은 선·뚜렷한 무늬와, 인식 뒤 나타날 요소·움직임을 화살표로 표시하세요.", width, 108 * mm, st)]
    elif lesson_id == 6:
...
def activity_answer_examples(era, lesson):
    ...
    if is_three:
        data_examples = {
            ...
            5: [
                ("체험 평가", "등장 요소·움직임·한 문장 해설·관람객 행동이 유산의 특징을 잘 보여 주는지 살펴봅니다."),
                ("등장 요소", "예: 백제 금동대향로 카드 위에 향로의 산 모양 뚜껑과 주요 무늬가 크게 나타납니다."),
                ("움직임과 해설", "천천히 회전하며 중요한 무늬를 차례로 가리키고, 확인한 사실 한 가지만 짧게 설명합니다."),
                ("관람객 행동", "관람객은 궁금한 부분을 눌러 확대하거나 다음 설명을 선택합니다."),
                ("구현 가능성", "40분 안에 카드 인식·한 가지 움직임·한 문장 해설부터 구현하고, 출처가 불확실한 장면은 넣지 않습니다."),
            ],
