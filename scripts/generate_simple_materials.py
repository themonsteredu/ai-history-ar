...
def student(plan, path, group=None):
    ...
    if i == 2:
        ...
        s.task(1)
        s.box('지난 활동지 ____번  /  고친 말의 핵심: ______________________\n\n더 알아보고 싶은 것: ____________________________________', 31)
        s.task(2)
        s.table(['문장', '핵심 낱말 (긴 문장은 화면에)', '자료 번호'],
                [[str(n), '', '자료 ____'] for n in range(1, 4)], [16, 128, 38], height=22)
        s.task(3)
        s.box('____번 문장은 __________________에서 찾았어요.\n\n□ 친구에게 표 한 줄을 설명했어요.    □ 오늘 작업을 저장했어요.', 29)
    elif i == 5:
        s.task(1)
        s.box('□ 같은 문장이 두 번 있는지 봤어요.\n□ 비슷한 내용끼리 묶었어요.\n□ 자료를 찾은 곳과 확인 상태를 봤어요.', 31)
        s.task(2)
        s.box('고친 곳: □ 같은 문장  □ 내용 묶기  □ 출처  □ 확인 상태\n\n______________________ → ______________________\n\n고칠 곳이 없다면: □ 다시 읽고 확인했어요.', 43)
        s.task(3)
        s.box('우리 표에 남은 문장: __________개\n\n□ 표 확인 끝!    □ 고친 표 파일 저장    □ 오늘 작업 저장', 32)
        s.note('모르는 내용은 지어 쓰지 않고 ‘판단 보류’로 남겨요.')
    elif i == 6:
        s.task(1)
        s.box('나누어 셀 것: □ 문장 종류 (살펴본항목)  □ 확인 상태\n\n전체 문장: __________개', 28)
        s.task(2)
        s.box('그래프 제목: __________________________________________\n\n□ 표 열기    □ 가로축에 항목 놓기    □ 문장 수 표시', 32)
        s.task(3)
        s.table(['비교할 종류', '표에서 센 수', '그래프의 수'], [['', '______개', '______개']], [94, 44, 44], 24)
        s.box('□ 두 개수가 같아요.   □ 다르면 그래프를 다시 살펴봐요.\n\n□ 그래프 가져오기    □ 오늘 작업 저장', 29)
        s.note('그래프는 우리가 고른 문장 수를 보여 줘요.')
    elif i == 7:
        s.task(1)
...
ASSESSMENT = {
    ...
    6: ['문장 종류 또는 확인 상태 중 하나를 골랐는가?', '우리 모둠 표로 그래프를 만들고 제목을 붙였는가?', '실제 개수가 일치하고 그래프가 작업 파일에 담겼는가?'],
